...
Lemma a_endmax_list (max: nat) (l : list nat) :
...
(*   intros. *)
  ...
  + induction l;intros.
    ...
    - case_eq l.
      * intros.
        assert (length (@nil nat) = 0);cbn;auto.
        rewrite H1 in *.
        simpl in H0.
        apply H0.
        apply a.
      * intros.
        ...
        simpl length in H3.
        simpl length in H0.
        replace (a0 :: l) with ([a0] ++ (l)) in H0.
        replace (S (length l) - 1 - 1) with (length l - 1) in H3.
        rewrite <- H3.
        apply H0.
        cbn.
        rewrite H1.
    ...
    + assert (Hamin : a = min).
      ...
      - intros.
        ...
        * rewrite Nat.ltb_lt in H.
          ...
          apply a_endmax_list in Hsmax; auto.
(*           case_eq (S min <? (max - 1)).
          2 : {
            intros.
        ...
        * rewrite Nat.ltb_nlt in H.
          ...
          rewrite H1.
          assert (l = []).
          apply a_endmax_list_ord with min;auto.
          rewrite H2.
          intros s Hs.
          auto.
Qed.
...
Theorem foreach_rule_plus (P : nat -> St -> Prop) (body : nat -> State () ):
  ...
  (forall (it : nat), {{fun s => P it s /\ (min <= it < max)}} body it {{fun _ => P (S it)}})
  ->
    {{P min}} foreach3 min max (fun it0 => loopeT_liftT (body it0)) {{fun _ => P max}} .
  Proof.
  unfold foreach3.
  intros.
  apply foreach_rule_plus_inter;auto.
  apply seq_ord.
  split.
  apply seq_min;auto.
  apply seq_max;auto.
